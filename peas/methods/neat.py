...
class NEATGenotype(object):
    ...
    def __init__(self, 
        ...
        if topology is None:
            ...
            for fr, to in topology:
                ...
                innov += 1
        #self.pyro_matrix = pyro_matrix(self)
    ...
    def pyrosim_matrix(self):
        """Returns weight matrix for use in pyrosim
        """
        # Initialize weight matrix:
        weight_matrix = np.zeros((self.inputs, self.outputs))
        for connection in sorted(self.conn_genes.keys()):
            # Because outputs are numbered range(self.inputs, self.inputs+self.outputs) we have
            # to subtract the number of inputs from the column index to get correct placement.
            # Further, self.conn_genes[connection][3] indexes the strength of the weight between
            # input and output.
            weight_matrix[connection[0], connection[1]-self.inputs] = self.conn_genes[connection][3]
